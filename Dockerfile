...
RUN yum install -y zsh mosh tmux openssh-server \
...
# Dev software
## Golang
RUN curl -sSLo go.tgz https://redirector.gvt1.com/edgedl/go/go1.9.2.linux-amd64.tar.gz; \
	tar -C /usr/lib/ -xzf go.tgz; \
    mv /usr/lib/go /usr/lib/golang; \
	rm go.tgz;
